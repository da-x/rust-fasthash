...
static U32 localfasthash_XXH32(const void* buffer, size_t bufferSize, U32 seed) { return fasthash_XXH32(buffer, bufferSize, seed); }

static U32 localfasthash_XXH64(const void* buffer, size_t bufferSize, U32 seed) { return (U32)fasthash_XXH64(buffer, bufferSize, seed); }
...
static void BMK_testfasthash_XXH32(const void* sequence, size_t len, U32 seed, U32 Nresult)
{
    XXH32_state_t state;
    size_t pos;

    BMK_checkResult32(fasthash_XXH32(sequence, len, seed), Nresult);

    (void)fasthash_XXH32_reset(&state, seed);
    (void)fasthash_XXH32_update(&state, sequence, len);
    BMK_checkResult32(fasthash_XXH32_digest(&state), Nresult);

    (void)fasthash_XXH32_reset(&state, seed);
    for (pos=0; pos<len; pos++)
        (void)fasthash_XXH32_update(&state, ((const char*)sequence)+pos, 1);
    BMK_checkResult32(fasthash_XXH32_digest(&state), Nresult);
}

static void BMK_testfasthash_XXH64(const void* data, size_t len, U64 seed, U64 Nresult)
{
    XXH64_state_t state;
    size_t pos;

    BMK_checkResult64(fasthash_XXH64(data, len, seed), Nresult);

    (void)fasthash_XXH64_reset(&state, seed);
    (void)fasthash_XXH64_update(&state, data, len);
    BMK_checkResult64(fasthash_XXH64_digest(&state), Nresult);

    (void)fasthash_XXH64_reset(&state, seed);
    for (pos=0; pos<len; pos++)
        (void)fasthash_XXH64_update(&state, ((const char*)data)+pos, 1);
    BMK_checkResult64(fasthash_XXH64_digest(&state), Nresult);
}
...
static void BMK_sanityCheck(void)
{
    ...
    BMK_testfasthash_XXH32(NULL,          0, 0,     0x02CC5D05);
    BMK_testfasthash_XXH32(NULL,          0, prime, 0x36B78AE7);
    BMK_testfasthash_XXH32(sanityBuffer,  1, 0,     0xCF65B03E);
    BMK_testfasthash_XXH32(sanityBuffer,  1, prime, 0xB4545AA4);
    BMK_testfasthash_XXH32(sanityBuffer, 14, 0,     0x1208E7E2);
    BMK_testfasthash_XXH32(sanityBuffer, 14, prime, 0x6AF1D1FE);
    BMK_testfasthash_XXH32(sanityBuffer,222, 0,     0x5BD11DBD);
    BMK_testfasthash_XXH32(sanityBuffer,222, prime, 0x58803C5F);

    BMK_testfasthash_XXH64(NULL        ,  0, 0,     0xEF46DB3751D8E999ULL);
    BMK_testfasthash_XXH64(NULL        ,  0, prime, 0xAC75FDA2929B17EFULL);
    BMK_testfasthash_XXH64(sanityBuffer,  1, 0,     0xE934A84ADB052768ULL);
    BMK_testfasthash_XXH64(sanityBuffer,  1, prime, 0x5014607643A9B4C3ULL);
    BMK_testfasthash_XXH64(sanityBuffer,  4, 0,     0x9136A0DCA57457EEULL);
    BMK_testfasthash_XXH64(sanityBuffer, 14, 0,     0x8282DCC4994E35C8ULL);
    BMK_testfasthash_XXH64(sanityBuffer, 14, prime, 0xC3BD6BF63DEB6DF0ULL);
    BMK_testfasthash_XXH64(sanityBuffer,222, 0,     0xB641AE8CB691C174ULL);
    BMK_testfasthash_XXH64(sanityBuffer,222, prime, 0x20CB8AB7AE10C14AULL);
...
static void BMK_hashStream(void* xxhHashValue, const algoType hashType, FILE* inFile, void* buffer, size_t blockSize)
{
    ...
    /* Init */
    (void)fasthash_XXH32_reset(&state32, XXHSUM32_DEFAULT_SEED);
    (void)fasthash_XXH64_reset(&state64, XXHSUM64_DEFAULT_SEED);
    ...
    while (readSize) {
        ...
        {
        case algo_xxh32:
            (void)fasthash_XXH32_update(&state32, buffer, readSize);
            break;
        case algo_xxh64:
            (void)fasthash_XXH64_update(&state64, buffer, readSize);
            break;
        default:
            break;
        }
    ...
    {
    case algo_xxh32:
        {   U32 const h32 = fasthash_XXH32_digest(&state32);
            memcpy(xxhHashValue, &h32, sizeof(h32));
            break;
        }
    case algo_xxh64:
        {   U64 const h64 = fasthash_XXH64_digest(&state64);
                    ...
                    const endianess displayEndianess)
{
    ...
    case algo_xxh32:
        {   XXH32_canonical_t hcbe32;
            (void)fasthash_XXH32_canonicalFromHash(&hcbe32, h32);
    ...
    case algo_xxh64:
        {   XXH64_canonical_t hcbe64;
            (void)fasthash_XXH64_canonicalFromHash(&hcbe64, h64);
            displayEndianess==big_endian ?
        ...
        if (fp == NULL) {
            ...
            case 32:
                {   XXH32_hash_t xxh;
                    ...
                    if (xxh == fasthash_XXH32_hashFromCanonical(&parsedLine.canonical.xxh32)) {
            ...
            case 64:
                {   XXH64_hash_t xxh;
                    ...
                    if (xxh == fasthash_XXH64_hashFromCanonical(&parsedLine.canonical.xxh64)) {
                        lineStatus = LineStatus_hashOk;
                }   }
                break;
